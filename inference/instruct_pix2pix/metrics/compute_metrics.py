...
class ImageEditor(nn.Module):
    ...
    def forward(
        ...
        with torch.no_grad(), autocast("cuda"), self.model.ema_scope():
            ...
            x = K.sampling.sample_euler_ancestral(
                self.model_wrap_cfg, x, sigmas, extra_args=extra_args
            )
...
def compute_metrics(
    ...
    outpath = Path(
        output_path, f"n={num_samples}_p={split}_s={steps}_r={res}_e={seed}.jsonl"
    )
    Path(output_path).mkdir(parents=True, exist_ok=True)

    for scale_txt in scales_txt:
        for scale_img in scales_img:
            dataset = EditDatasetEval(path=data_path, split=split, res=res)
